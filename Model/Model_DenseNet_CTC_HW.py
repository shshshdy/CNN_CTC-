...
import Model.densenet_HW as densenet
import os
...
class Model_DenseNet_CTC_HW():
    ...
    def evaluate(self,base_model,batch_num=10):
        ...
        for i in range(batch_num):
            [y_test,X_test, _, _], _ = next(data.gen_data_by_batch_ctc_yield_hw(batch_size=size,
            ...
            # out like [ 5  1  8  5  0  0 -1 -1]
            # print('out:' + str(out))
            for i in range(len(X_test)):
                str_src = y_test[i]
                # str_src=[ 2  6  5  9  5  2  6 11 11 11 11]
                # str_src = [2, 6, 5, 9, 5, 2, 6, 11, 11, 11, 11]
                x = '@'.join(str(e) for e in str_src)
                str_src = x.replace('11', '').replace('@', '')
                # str_src = 2659526
                ...
                if str_src != str_out:
                    ...
                    print('str_src={}  str_out={}'.format(str_src, str_out))
        a = (batch_num*size - error_count)
        b = (batch_num*size)
        c = a/b
        result = float((batch_num*size - error_count) / (batch_num*size))
        return result
...
def predict():
    ...
    data_val = DataLoad_Model_by_Path(val_path)
    a_model.build_model()
    a_model.load_model_weight('../saved_models-17092018-150022-e100.h5')
    result = a_model.evaluate(a_model.basemodel,batch_num=10)
    return result
